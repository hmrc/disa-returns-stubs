...
|         Disallowed MIME type            | the uploaded file's `Content-Type` is not `text/csv`, `application/vnd.ms-excel` or `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` | a `.pdf` file uploaded with `Content-Type: application/pdf` | Intercepted by this stub, which mimics real upscan: the request is redirected to `success_action_redirect` (with `key`), and a `FAILED`/`REJECTED` callback is sent asynchronously to `x-amz-meta-callback-url` |
