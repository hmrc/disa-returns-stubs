...
class EtmpController @Inject() (
  ...
  def checkReportingWindowStatus: Action[AnyContent] = Action.async {
    reportingWindowRepository.getReportingWindowState.map {
      case Some(open) =>
        logger.info(s"[EtmpController][checkReportingWindowStatus] ETMP Reporting Window isOpen: $open")
        Ok(Json.toJson(EtmpReportingWindow(reportingWindowOpen = open)))
      case None       =>
        logger.warn(s"[EtmpController][checkReportingWindowStatus] No reporting window state found")
        NotFound(Json.obj("error" -> "No reporting window state found"))
    }
  }

  def declare(zReference: String): Action[AnyContent] = Action.async {
    logger.info(s"[EtmpController][declare] Declaration received for IM ref: [$zReference], closing obligation status")
  ...
  def checkReturnsObligationStatus(zReference: String): Action[AnyContent] = Action.async {
    obligationStatusRepository
      ...
      .flatMap {
        case Some(status) =>
          logger.info(
            s"[EtmpController][checkReturnsObligationStatus] Return obligation status for IM ref: [$zReference] with status: [$status]"
          )
          Future.successful(Ok(Json.toJson(EtmpObligations(obligationAlreadyMet = status))))
        case None         =>
          logger.info(
            s"[EtmpController][checkReturnsObligationStatus] Return obligation status for IM ref: [$zReference] with status: [false]"
          )
          obligationStatusRepository
